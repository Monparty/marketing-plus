...
export function Process() {
  return (
    <section id="process" className="bg-surface px-5 py-section">
      <div className="mx-auto max-w-[1200px]">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(230px,1fr))] gap-5">
          {process.items.map((st) => (
            <Reveal
              key={st.n}
              className="relative rounded-card border border-line bg-white px-6 py-7"
            >
              <div className="mb-4 flex items-center gap-3">
                ...
                <span className="h-px flex-1 bg-[linear-gradient(90deg,#DCE3EF,rgba(220,227,239,0))]" />
                <span
                  aria-hidden="true"
